...
public class MyPublicKeyAuthenticator implements PublickeyAuthenticator {
    ...
    public boolean authenticate(String username, PublicKey key, ServerSession session) throws AsyncAuthException {
        ...
        if (userOpt.isEmpty()) {
            ...
            saveAuthLog(sessionId, username, clientAddress, SftpOperationLogStatus.ERROR, "用户不存在: " + username);
            return false;
        }
        ...
        if (!user.getEnabled()) {
            ...
            saveAuthLog(sessionId, username, clientAddress, SftpOperationLogStatus.ERROR, "用户已禁用: " + username);
            return false;
        }
        ...
        if (storedPublicKey == null || storedPublicKey.isBlank()) {
            ...
            saveAuthLog(sessionId, username, clientAddress, SftpOperationLogStatus.ERROR, "用户未配置公钥: " + username);
            return false;
        }
        ...
        try {
            ...
            saveAuthLog(sessionId, username, clientAddress, SftpOperationLogStatus.ERROR, "公钥解析失败: " + username);
            return false;
        }

        log.warn("Public key authentication failed: key mismatch - {}", username);
        saveAuthLog(sessionId, username, clientAddress, SftpOperationLogStatus.ERROR, "公钥不匹配: " + username);
        return false;
    }

    private void saveAuthLog(String sessionId, String username, String clientAddress, SftpOperationLogStatus status, String description) {
        try {
