...
public class MyPasswordAuthenticator implements PasswordAuthenticator {
    ...
    public boolean authenticate(String username, String password, ServerSession session) throws PasswordChangeRequiredException, AsyncAuthException {
        ...
        if (user.getPasswordLogin() != null && !user.getPasswordLogin()) {
            ...
            saveAuthLog(sessionId, username, clientAddress, SftpOperationLogStatus.ERROR, "密码登录已禁用: " + username);
            return false;
        }

        if (!verifyPassword(password, user.getPassword())) {
            log.warn("Password authentication failed: invalid password - {}", username);
            saveAuthLog(sessionId, username, clientAddress, SftpOperationLogStatus.ERROR, "密码错误: " + username);
            return false;
        }
