<template>
  <div class="container">
    <el-card v-loading="loading">
      <template #header><h2 style="margin:0;">通信主体</h2></template>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit?'编辑通信主体':'新增通信主体'" width="80%">
...
async function load(){loading.value=true;try{const{data}=await rosettanetApi.personalities.list();items.value=data.data||[]}finally{loading.value=false}}
...
async function handleSave(){saveLoading.value=true;try{if(isEdit.value){await rosettanetApi.personalities.update(editingId.value,form.value)}else{await rosettanetApi.personalities.create(form.value)}ElMessage.success('保存成功');dialogVisible.value=false;await load()}catch(e){ElMessage.error(e.response?.data?.message||e.message)}finally{saveLoading.value=false}}
async function handleDelete(r){try{await ElMessageBox.confirm(`确定删除 ${r.name}？`,'确认',{type:'warning'});await rosettanetApi.personalities.delete(r.id);ElMessage.success('删除成功');await load()}catch(e){if(e!=='cancel')ElMessage.error('删除失败')}}
onMounted(load)
</script>
